...
public class FoodPresenter {
    ...
    public void confirmOrder(float newTotalPrice, String selectedPaymentMethod) {
        boolean orderConfirmed = food.confirmOrder(newTotalPrice, selectedPaymentMethod);

        if (orderConfirmed) {
            view.showOrderConfirmationSuccess(selectedPaymentMethod);
        } else {
